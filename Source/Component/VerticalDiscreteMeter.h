...
namespace Gui
{
	...
	public:
		...
		void paint(Graphics& g) override
		{
			...
			for (auto i = 0; i < totalNumberOfBubls; i++)
			{
				if (level >= static_cast<float>(i + 1) / totalNumberOfBubls)
					bulbs[i]->setState(true);
				else
					bulbs[i]->setState(false);

				// This can also be written as
				// bulbs[i]->setState(level >= static_cast<float>(i + 1) / totalNumberOfBubls)
			}
		...
		void resized() override
		{
			...
			const auto bulbHeight = bulbBounds.getHeight() / totalNumberOfBubls;
			bulbs.clear();
			for (auto i = 0; i < totalNumberOfBubls; i++)
			{
				auto bulb = 
					std::make_unique<Bulb>(gradient.getColourAtPosition(static_cast<double>(i) / totalNumberOfBubls));
	...
	private:
		...
		const int totalNumberOfBubls = 10;
	};
}
